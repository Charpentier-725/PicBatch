...
**核心文件**:
- `src/lib/analytics.ts` (263行) - 统计数据管理
- `src/lib/feedback.ts` (146行) - 反馈消息系统
- `src/components/StatsDialog.tsx` (235行) - 统计UI
- `src/components/ui/separator.tsx` - UI分隔组件

**集成点**:
- `Header.tsx` - 添加统计按钮图标
- `App.tsx` - 自动追踪用户会话
- `useImageProcessor.ts` - 记录处理数据和优化提示
