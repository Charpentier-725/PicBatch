...
| 配置项 | 你填的 | 状态 | 说明 |
|--------|--------|------|------|
| **Framework preset** | `None` | ✅ 正确 | 或选择 `Vite`（会自动填充命令） |
| **Build command** | `npm run build` | ✅ 正确 | 构建生产版本的命令 |
| **Build output directory** | `dist` | ✅ 正确 | **必须是 `dist`！** |
| **Root directory** | （空） | ✅ 正确（保持默认） | 项目根目录，留空即可 |
| **Environment variables** | `NODE_VERSION = 18` | ✅ 完美！ | 确保使用 Node.js 18 版本构建 |
...
| 字段 | 填什么 | 说明 |
|------|--------|------|
| **Project name** | `picbatch` | 网址会是 `picbatch.pages.dev`，可自定义 |
| **Production branch** | `main` | 选择主分支（默认已选好） |

---

**② 构建设置区域**（Build settings）:

| 字段 | 填什么 | 重要性 |
|------|--------|--------|
| **Framework preset** | `None` 或 `Vite` | 推荐选 `None` 手动配置 |
| **Build command** | `npm run build` | ⭐ 必填 |
| **Build output directory** | `dist` | ⭐⭐⭐ 最重要！必须是 `dist` |
| **Root directory** | 留空 | 可选，保持默认 |

---

**③ 环境变量区域**（Environment variables - 可选）:
